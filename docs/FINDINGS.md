...
## 二、Phase 2 事件演练发现（2026-07-30，未跑完）

> 场景独立管理于 [scenarios/phase2-loadtest/](../scenarios/phase2-loadtest/)（复盘表在其 README）。
> 本次只跑到"处置前（load，无 HPA）基线"，**remediate 处置对比未做**。

**17. 演练要走完事件生命周期，别把"问题"和"解药"绑一起**
- 原 `run.sh` 一步同时"部署 HPA + 加压"，看不到"无自愈"时系统烂成什么样。
- 改为分阶段 Game Day：`load`（制造事故，**故意不装 HPA**）→ 分析 → `remediate`（装 HPA）→ 恢复 → `stop`。
- 结论：能量化"处置前 vs 处置后"才是能写进复盘的对照数据。

**18. 聚合指标骗人，瓶颈藏在写路径**
- 6 副本压测：聚合中位数仅 73ms 看着"健康"，但 `POST /login` 中位 7500ms/峰值 27s、`POST /signup` 失败率 37%。
- 读路径（GET /、/home）全程正常，**崩的是写路径**；瓶颈精确定位在 userservice（CPU 94%，登录/注册都走它）。
- 结论：必看**逐接口 + 百分位**，只看聚合/平均会漏掉灾难。

**19. 症状 vs 原因：人肉抽查会骗人**
- 手动测两次得相反结论：一次开户成功（"没问题啊"），一次 `Error: Account creation failed`（正好落进那 37%）。
- 单次体验是抽样，会骗人；只有持续测量 + 聚合统计 + 百分位才见真相（"稳定 37% 用户开户失败"）。
- 印证 SRE 铁律：**对症状/SLO 告警（开户成功率跌破 SLO），不对 CPU 告警**。当前监控只有资源型（CPU/重启）信号，缺 Latency/Errors/SLO —— 正是 O1（L7 Ingress）+ GMP 要补的。

**20. 饱和平台期（saturation plateau）**
- replicas 6→10 加压 66%，失败率仍锁死 ~37%、延迟持平。
- 机制：瓶颈是固定产能的单副本 userservice，饱和后加压只是排更长队 + 更多受害者，单请求体验不再恶化。
- 结论：解法是**加产能（HPA 横向扩容）**，不是减载。

**21. 测量陷阱：累计计数器 + 大数定律稀释新信号（⚠️ 未收尾）**
- `kubectl scale 6→10` 不重启旧 pod；`head -1` 读到长跑老 pod 的**累计**计数器（已 2939 次），新负载摊进大分母 → 变化几乎看不出，"37% 稳定"部分是**测量假象**。
- 干净重测法：`kubectl rollout restart deployment loadgenerator`（Locust 计数清零，保持副本=10）→ 跑 3-5min 收敛 → 再读。且 `head -1` 仅 1 个 pod，严谨应多读几个。
- **待验证**：清零后 replicas=10 是否仍收敛到 ~37%（证实平台期）还是更高（说明仍在恶化）。

**22. 37% 失败率的成因假说（未验证）**
- 假说：signup 是最重写操作（bcrypt 哈希 + 写库 + 自动登录），饱和时延迟分布被某个**固定超时线**切出约 37% 落在右侧 → 失败；login 更轻，分布整体在超时线左 → 只慢（0% 失败）。
- **待验证**：查 frontend/userservice 日志，确认失败是 timeout / 500 / 连接拒，并找出超时值。

---

## 三、待补（各 Phase 跑完追加）

- [x] Phase 2 压测/扩容：见上"二、Phase 2 事件演练发现"（处置对比 + 清零重测仍待做）
