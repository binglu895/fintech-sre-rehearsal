...
| 2026-07-30 | 处置前(load,无HPA) | 6 | 待确认 | 全 1 | 3 | 聚合 1233/73/27104 | 1.08% | 否 | 事故基线：POST /login Med 7500ms、POST /signup 失败率 37% |
| | 处置后(remediate,有HPA) | 6 | | | | | | | 扩容后恢复 |

> 处置前关键发现（逐接口）：聚合中位数只有 73ms 看着"正常"，但写路径彻底崩了——
> `POST /login` 中位 7500ms / 峰值 27 秒，`POST /signup` 失败率 37%。瓶颈在 userservice（CPU 94%）。
>
> 饱和平台期：replicas 6→10 加压 66%，失败率仍锁死 37%、延迟持平——瓶颈是固定产能的
> 单副本 userservice，加压只是排更长队 + 更多受害者，症状不变。结论：解法是加产能(HPA)，非减载。
